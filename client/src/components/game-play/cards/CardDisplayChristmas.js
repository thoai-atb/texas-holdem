...
import cardBackChristmas from "../../../assets/texture/card-back-christmas.png";
import { CardSuit } from "../../../utilities/card";

export function CardDisplayChristmas({ card, hidden, showDown, highlight }) {
  return (
    <div
      className={
    ...
    >
      {card && !hidden && (
        ...
        >
          ...
          <div className="absolute bottom-1 -right-8 clear-both">
            <div className="text-6xl">{getSymbol(card.suit)}</div>
          </div>
        </div>
      )}
      {hidden && (
        <div
          ...
          style={{
            backgroundImage: `url(${cardBackChristmas})`,
