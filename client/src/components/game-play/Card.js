...
import cardBackHalloween from "../../assets/texture/card-back-halloween.jpg";
import { getHalloweenSymbol } from "../../utilities/card";
...
export function CardDisplayHalloween({ card, hidden, showDown, highlight }) {
  return (
    <div
      className={
        "transition duration-500 w-16 h-24 m-0.5 flex justify-center items-center select-none rounded-lg relative" +
    ...
    >
      {card && !hidden && (
        <div className={["♥", "♦"].includes(card.suit) ? " text-red-600" : " text-black"}>
          ...
          <div className="absolute bottom-1 -right-7 clear-both">
            <div className="text-6xl">{getHalloweenSymbol(card.suit)}</div>
          </div>
        </div>
      )}
      {hidden && (
